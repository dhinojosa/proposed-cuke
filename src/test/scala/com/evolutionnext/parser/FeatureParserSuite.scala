...
import com.evolutionnext.gherkin.FeatureChild.ScenarioCase
import com.evolutionnext.gherkin.{Feature, FeatureChild, Tag}
...
class FeatureParserSuite extends munit.FunSuite {
  ...
  test("read in 000-basic-scenario.feature") {
    ...
    assertEquals(feature.children.length, 1)

    val featureChild: FeatureChild = feature.children.head
    featureChild match {
  ...
  test("basic parse with annotation") {
    ...
    assertEquals(feature.name, "Basic arithmetic")
    assert(feature.children.nonEmpty)

    val head = feature.children.head
    head match {
      case ScenarioCase(scenario) =>
        assertEquals(scenario.name, "Add two numbers")
        assertEquals(scenario.steps.size, 4)
      case _ => fail("Scenario Not Found")
    }
  }
  ...
  test("basic parse with two annotations and two scenarios") {
    ...
    assertEquals(feature.name, "Basic arithmetic")
    assert(feature.children.nonEmpty)
    assertEquals(feature.children.length, 2)
  }
  ...
  test("basic parse with two annotations and two scenarios, tags on the first") {
    ...
    feature.children.head match {
      ...
      case _ => fail("Could not find the scenario")
    }
  }
  ...
  test("parse 003-scenario-outline.feature") {
    val io: IO[String] =
      Resource
        .make(IO(Source.fromResource("003-scenario-outline.feature")))(source =>
          IO(source.close()))
        .use(source => IO(source.mkString))
    val eitherFeature: Either[Parser.Error, Feature] =
      io.map(FeatureParser.parse).unsafeRunSync()
